...
--SELECT 12
SELECT SUM(quantProduto) AS 'Soma da quantd. de amaciantes:' FROM tbProduto
...
--SELECT 16
SELECT AVG(quantItensVenda) AS 'Média de itens por venda:' FROM tbItensVenda
GROUP BY codVenda
...
--SELECT 18
SELECT SUM(quantProduto) AS 'Soma das quantidades de produtos que ultrapassam 800 unidades:' FROM tbProduto
WHERE quantProduto > 800

--SELECT 19
SELECT AVG(valorProduto) AS 'Média dos preços da Unilever com mais de 1000 unidades:' FROM tbProduto
WHERE codFabricante = 1 AND quantProduto > 1000
